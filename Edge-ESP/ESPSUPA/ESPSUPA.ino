...
void setup() {
  ...
  // Bắt đầu BLE Provisioning trong 20 giây
  WiFiProv.beginProvision(WIFI_PROV_SCHEME_BLE, WIFI_PROV_SCHEME_HANDLER_FREE_BTDM, WIFI_PROV_SECURITY_1, "abcd1234", "PROV_CAM01");
  ...
  while (millis() - startMillis < 20000) {
    if (WiFi.status() == WL_CONNECTED) {
      ...
      // Tắt BLE sau khi kết nối Wi-Fi thành công
  ...
  // Nếu không kết nối được trong 20 giây, sử dụng thông tin Wi-Fi mặc định
  Serial.println("BLE Provisioning Timeout, Connecting to default Wi-Fi...");
